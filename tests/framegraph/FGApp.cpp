...
*/
	...
	{
		_frameGraph->ResizeSurface( size );
	}
...
*/
	bool FGApp::_Initialize (WindowPtr &&wnd)
	{
		...
		{
			_vulkan.Create( _window->GetVulkanSurface(), "Test", "FrameGraph" );
		...
		VulkanDeviceInfo	vulkan_info;
		{
			...
			vulkan_info.surface			= BitCast<VkSurface_t>( _vulkan.GetVkSurface() );
		...
		{
			_frameGraph = FrameGraph::CreateVulkanFrameGraph( vulkan_info );
			CHECK_ERR( _frameGraph );

			CHECK_ERR( _frameGraph->Initialize( 2 ) );

			_frameGraph->SetCompilationFlags( ECompilationFlags::EnableDebugger );
		}
...
=================================================
*/
	BufferPtr  FGApp::_CreateBuffer (BytesU size, StringView name) const
	{
		BufferPtr	res = _frameGraph->CreateBuffer( EMemoryType::Default, size, EBufferUsage::Transfer | EBufferUsage::Uniform | EBufferUsage::Storage |
...
=================================================
*/
	ImagePtr  FGApp::_CreateImage2D (uint2 size, EPixelFormat fmt, StringView name) const
	{
		ImagePtr	res = _frameGraph->CreateImage( EMemoryType::Default, EImage::Tex2D, uint3(size.x, size.y, 0), fmt,
...
*/
	...
	{
		ImagePtr	res = _frameGraph->CreateLogicalImage( EMemoryType::Default, EImage::Tex2D, uint3(size.x, size.y, 0), fmt );
...
*/
	void FGApp::Run ()
	{
		...
		if ( not app._Initialize( std::move(wnd) ) )
			return;
