...
namespace FG
{
	...
	private:
		...
		ND_ BufferPtr		_CreateBuffer (BytesU size, StringView name = Default) const;

		ND_ ImagePtr		_CreateImage2D (uint2 size, EPixelFormat fmt = EPixelFormat::RGBA8_UNorm, StringView name = Default) const;
		ND_ ImagePtr		_CreateLogicalImage2D (uint2 size, EPixelFormat fmt = EPixelFormat::RGBA8_UNorm, StringView name = Default) const;
