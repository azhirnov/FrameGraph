...
namespace FG
{
	...
	{
	public:
		using Barrier = VBuffer::BufferBarrier;

		static ArrayView<Barrier>  GetReadBarriers (const VBufferPtr &buf) {
			return buf->_readBarriers;
		}
		
		static ArrayView<Barrier>  GetWriteBarriers (const VBufferPtr &buf) {
...
static void VBuffer_Test1 (const FrameGraphPtr &fg)
{
	...
	VBufferPtr	buf = Cast<VBuffer>( fg->CreateBuffer( EMemoryType::Default, 1024_b, EBufferUsage::All ));
