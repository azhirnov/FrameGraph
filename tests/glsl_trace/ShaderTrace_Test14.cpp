...
void main ()
{
	dbg_EnableTraceRecording( gl_LaunchIDNV.x == 60 && gl_LaunchIDNV.y == 40 );
...
void main ()
{
	dbg_EnableTraceRecording( gl_LaunchIDNV.x == 50 && gl_LaunchIDNV.y == 50 );
...
void main ()
{
	dbg_EnableTraceRecording( gl_LaunchIDNV.x == 70 && gl_LaunchIDNV.y == 80 );
...
extern bool ShaderTrace_Test14 (VulkanDeviceExt& vulkan, const TestHelpers &helper)
{
	...
	CHECK_ERR( CreateDebugDescriptorSet( vulkan, helper, VK_SHADER_STAGE_RAYGEN_BIT_NV | VK_SHADER_STAGE_CLOSEST_HIT_BIT_NV | VK_SHADER_STAGE_MISS_BIT_NV, OUT ds2_layout, OUT desc_set2 ));
	...
	{
		const uint	data[] = { UMax, UMax, UMax };
