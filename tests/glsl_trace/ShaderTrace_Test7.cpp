...
void main ()
{
	...
	out_Color   = vec4( in_Level[0], in_Level[1], in_Level[2], 1.0f );
		...
		CHECK_ERR( shaderCompiler.Compile( OUT tessEvalShader, vulkan, {eval_shader_source}, EShLangTessEvaluation ));
	}
...
extern bool ShaderTrace_Test7 (VulkanDeviceExt& vulkan, const TestHelpers &helper)
{
	...
	CHECK_ERR( CreateDebugDescriptorSet( vulkan, helper, VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT, OUT ds_layout, OUT desc_set ));
	...
	// debug output storage read/write after write
	{
		...
		vulkan.vkCmdPipelineBarrier( helper.cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TESSELLATION_CONTROL_SHADER_BIT, 0,
	...
	// debug output storage read after write
	{
		...
		vulkan.vkCmdPipelineBarrier( helper.cmdBuffer, VK_PIPELINE_STAGE_TESSELLATION_CONTROL_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0,
									 0, null, 1, &barrier, 0, null);
	}
