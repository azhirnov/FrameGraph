...
*/
	...
	{
		String	result;
		for (uint t = 1; t < VK_PIPELINE_STAGE_FLAG_BITS_MAX_ENUM; t <<= 1)
		{
...
*/
	...
	{
		String	result;
		for (uint t = 1; t < VK_DEPENDENCY_FLAG_BITS_MAX_ENUM; t <<= 1)
		{
			if ( not EnumEq( values, t ) )
				continue;
...
*/
	...
	{
		String	result;
		for (uint t = 1; t < VK_ACCESS_FLAG_BITS_MAX_ENUM; t <<= 1)
		{
			if ( not EnumEq( values, t ) )
				continue;
...
*/
	...
	{
		String	result;
		for (uint t = 1; t < VK_IMAGE_ASPECT_FLAG_BITS_MAX_ENUM; t <<= 1)
		{
			if ( not EnumEq( values, t ) )
				continue;
