...
=================================================
*/
	VTaskProcessor::VTaskProcessor (VFrameGraph &fg) :
		_frameGraph{ fg },
		_dev{ fg.GetDevice() },
		_currQueue{ _frameGraph.GetArrangedQueues().main }
	{
		_cmdBufferId = _currQueue->CreateCommandBuffer();
...
=================================================
*/
	forceinline void VTaskProcessor::_OnRunTask (VCommandQueue const* queue, const IFrameGraphTask *task) const
	{
...
=================================================
*/
	void VTaskProcessor::_BindPipelineResources (const VComputePipelinePtr &pipeline, const VPipelineResourceSet &resources) const
	{
		FixedArray< VkDescriptorSet, VPipelineResourceSet::capacity() >		descriptor_sets;
		descriptor_sets.resize( resources.size() );
		...
		for (const auto& res : resources)
		{
			uint	idx = pipeline->GetBindingIndex( res.first );
			ASSERT( idx < descriptor_sets.size() );

			descriptor_sets[idx] = res.second->GetDescriptorSetID();
		}

		_dev.vkCmdBindDescriptorSets( _cmdBufferId,
									  VK_PIPELINE_BIND_POINT_COMPUTE,
									  pipeline->GetLayout()->GetLayoutID(),
									  0,
									  uint(descriptor_sets.size()),
									  descriptor_sets.data(),
									  0, null );
...
=================================================
*/
	void VTaskProcessor::_BindPipeline (const VComputePipelinePtr &pipeline, const Optional<uint3> &localSize) const
	{
		VkPipeline	ppln_id = _frameGraph.GetPipelineCache().CreatePipelineInstance( pipeline, localSize, 0 );
...
*/
	void VTaskProcessor::Visit (const VFgTask<CopyBuffer> &task)
	{
		...
		VBufferPtr	src_buffer	= Cast<VBuffer>( task.SrcBuffer()->GetReal( _currTask, EResourceState::TransferSrc ));
		VBufferPtr	dst_buffer	= Cast<VBuffer>( task.DstBuffer()->GetReal( _currTask, EResourceState::TransferDst ));

		if ( task.SrcBuffer()->IsMutable() or task.DstBuffer()->IsMutable() )
		{
			for (auto& reg : task.Regions())
			{
				_AddBuffer( src_buffer, EResourceState::TransferSrc, reg.srcOffset, reg.size );
				_AddBuffer( dst_buffer, EResourceState::TransferDst, reg.dstOffset, reg.size );
			}
		}
		...
		_dev.vkCmdCopyBuffer( _cmdBufferId,
							  src_buffer->GetBufferID(),
							  dst_buffer->GetBufferID(),
							  uint( task.Regions().size() ),
							  task.Regions().data() );
	}
...
*/
	void VTaskProcessor::Visit (const VFgTask<CopyImage> &task)
	{
		...
		VImagePtr	src_image	= Cast<VImage>( task.SrcImage()->GetReal( _currTask, EResourceState::TransferSrc ));
		VImagePtr	dst_image	= Cast<VImage>( task.DstImage()->GetReal( _currTask, EResourceState::TransferDst ));

		for (auto& reg : task.Regions())
		{
			_AddImage( src_image, EResourceState::TransferSrc, task.SrcLayout(), reg.srcSubresource );
			_AddImage( dst_image, EResourceState::TransferDst, task.DstLayout(), reg.dstSubresource );
		}
		
		_CommitBarriers();
		
		_dev.vkCmdCopyImage( _cmdBufferId,
							 src_image->GetImageID(),
							 task.SrcLayout(),
							 dst_image->GetImageID(),
							 task.DstLayout(),
							 uint(task.Regions().size()),
							 task.Regions().data() );
	}
...
*/
	void VTaskProcessor::Visit (const VFgTask<CopyBufferToImage> &task)
	{
		...
		for (auto& reg : task.Regions())
		{
			_AddBuffer( src_buffer, EResourceState::TransferSrc, reg, dst_image );
			_AddImage(  dst_image,  EResourceState::TransferDst, task.DstLayout(), reg.imageSubresource );
		}
...
*/
	void VTaskProcessor::Visit (const VFgTask<CopyImageToBuffer> &task)
	{
		...
		for (auto& reg : task.Regions())
		{
			_AddImage(  src_image,  EResourceState::TransferSrc, task.SrcLayout(), reg.imageSubresource );
			_AddBuffer( dst_buffer, EResourceState::TransferDst, reg, src_image );
		}
...
*/
	void VTaskProcessor::Visit (const VFgTask<ClearColorImage> &task)
	{
		...
		for (auto& reg : task.Ranges())
		{
			_AddImage( dst_image, EResourceState::TransferDst, task.DstLayout(), reg );
		}
...
=================================================
*/
	inline void VTaskProcessor::_AddImageState (const VImagePtr &img, const ImageState &state)
	{
...
=================================================
*/
	inline void VTaskProcessor::_AddImage (const VImagePtr &img, EResourceState state, VkImageLayout layout, const ImageViewDesc &desc)
	{
...
=================================================
*/
	inline void VTaskProcessor::_AddImage (const VImagePtr &img, EResourceState state, VkImageLayout layout, const VkImageSubresourceLayers &subres)
	{
		_AddImageState( img,
						ImageState{
							state, layout,
...
=================================================
*/
	inline void VTaskProcessor::_AddImage (const VImagePtr &img, EResourceState state, VkImageLayout layout, const VkImageSubresourceRange &subres)
	{
		_AddImageState( img,
						ImageState{
							state, layout,
...
=================================================
*/
	inline void VTaskProcessor::_AddBufferState (const VBufferPtr &buf, const BufferState &state)
	{
...
=================================================
*/
	inline void VTaskProcessor::_AddBuffer (const VBufferPtr &buf, EResourceState state, VkDeviceSize offset, VkDeviceSize size)
	{
		if ( buf->IsImmutable() )
			return;
...
=================================================
*/
	inline void VTaskProcessor::_AddBuffer (const VBufferPtr &buf, EResourceState state,
											const VkBufferImageCopy &reg, const VImagePtr &img)
	{
		if ( buf->IsImmutable() )
			return;
