...
=================================================
*/
	bool VRenderPass::_Setup (ArrayView<VLogicalRenderPass*> logicalPasses, ArrayView<GraphicsPipelineDesc::FragmentOutput> fragOutput)
	{
		...
		for (auto& frag : fragOutput)
		{
			...
			desc.samples		= VEnumCast( iter->second.image->Description().samples );
		...
		if ( pass->GetDepthStencilTarget().IsDefined() )
		{
			...
			desc.samples		= VEnumCast( ds_target.image->Description().samples );
...
=================================================
*/
	bool VRenderPass::_Create (const VDevice &dev)
	{
		CHECK_ERR( _renderPass == VK_NULL_HANDLE );
...
=================================================
*/
	void VRenderPass::_Destroy (const VDevice &dev)
	{
		if ( _renderPass )
		{
